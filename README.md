...
definition of mutliple authorization links forming an authorization chain.
...
## Prerequisites
While this module provides authorization services, you are still required to
write your own authentication adapater that implements
`AuthenticationAdapterInterface` from `laminas/laminas-authentication`.  

Depending on what sort of Authentication Service you are intending to use, you
need to configure your session container (storage) that you define in this
module's configuration, e.g. if you specify the value `LoginAuth` for the
`session_container` key under this module's configuration, you need to specify
that container name in the `session_containers` key in laminas configuration
(preferably in the global configuration).  

In other words, if you reference any session container name, make sure to
reference it under the `session_containers` key in laminas config:
...
## Authorization Link
An Authorization Link is built around entities that can infer whether an
...
and  a `AccessControlListInterface` from `alichry/laminas-accesscontrol`
that defines a list of properties that dictates the roles, permissions and the
accessibility or authorization level of each controller or controller's action.  
...
While most applications generally utilizes only one link, this is primarily
...
ACL configuration defines a list of controller/action entries that can indicate
the authorization level it requires. Multiple implementations of
`AccessControlListInterface` from `alichry/laminas-accesscontrol` exists, the
`ArrayAccessControlList` provides a simple array-based ACL. For latter
implementation, there exists the following statuses or levels:  
...
Static functions provided by `ArrayAccessControlList` can aid in building your
configuration and ensures compatibility with future versions.
See
...
module, then register it under the module's config. 
...
under `authorization` key in the configuration. You can place that key in the 
lobal configuration (config/autoload/global.php). As we mentioned earlier,
...
where each item in the array consists of link options mapped by a link name key.
The link options are pertaining the redirect route to utilize during filtering
unauthorized users, the Authentication Service and the Access Control List adapter
...
requires build options, you need to provide build options under 
...
option. We need to specify which `AccessControlListInterface` to use and pass any
build option if it is required. The Access Control module, from
`alichry/laminas-accesscontrol`, provides a factory that builds an
`ArrayAccessControlList` by relying on passed build options.  
                    ...
                    'options' => [
                        ...
                        'controllers' => [
                            ...
                            YourMultipleAccessController::class => [
                                ...
                                'adminPerm' => ACL::permission('admin-perm'),
